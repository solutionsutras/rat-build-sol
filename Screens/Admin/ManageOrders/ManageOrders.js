...
const ManageOrders = (props) => {
  ...
  const changeStatus = (changedStatus) => {
    ...
    const changedOrder = {
      ...
      advanceToPay: orders.advanceToPay,
      advancePaid: orders.advancePaid,
      balanceToPay: orders.balanceToPay,
      user: orders.user.id,
      dateOrdered: orders.dateOrdered,
    };

    // console.log('changedStstus: ', changedStatus);

    if (changedStatus === '10' || changedStatus === '20') {
      axios
        .put(`${baseUrl}orders/${orders.id}`, changedOrder, config)
        .then((res) => {
  ...
  const sendSms = () => {
    console.log('Send SMS ACCEPTED');
  };

  return (
    <View style={[{}, styles.container]}>
      <View style={[styles.statusContainer, { backgroundColor: cardColor }]}>
      ...
      {statusText === 'pending' ? (
        <View>
          <View style={styles.buttonContainer}>
            <EasyButton large secondary onPress={() => changeStatus('10')}>
              <Text style={{ color: 'white', fontWeight: 'bold' }}>
                Accept Order
              </Text>
            </EasyButton>
      ...
      {statusText === 'accepted' ? (
        <View>
          <View style={styles.buttonContainer}>
            <EasyButton large secondary onPress={() => changeStatus('20')}>
              <Text style={{ color: 'white', fontWeight: 'bold' }}>
                Start Processing
              </Text>
            </EasyButton>
